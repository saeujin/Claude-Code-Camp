/**
 * F1. 사용자 프로필 및 목표 설정 — 명세 85~199줄의 계산.
 *
 * 출처: JMS 브랜치의 `lib/nutrition/calculate.ts`. 계산 로직은 그대로 옮겼고,
 * 날짜 유틸만 중복을 피해 이 저장소의 `./date`를 쓰도록 바꿨다.
 *
 * ⚠️ 반올림 시점이 결과를 바꾼다.
 *    명세 계산 예시 ㉮는 TDEE와 일일 조정량을 **각각 반올림한 뒤** 빼서 1,581을 얻는다.
 *      round(2038.5) − round(458.33) = 2039 − 458 = 1581   ✓ 명세 값
 *      round(2038.5 − 458.33)        = round(1580.17) = 1580   ✗
 *    단계별 반올림을 유지할 것.
 *
 * ⚠️ 기본 목표 칼로리는 어떤 경우에도 보정하지 않는다.
 *    BMR보다 낮아도, 0 이하여도 계산값 그대로 돌려준다.
 *    명세 130~133줄: "앱은 값을 제한하지 않는다 … 속도를 정하는 것은 사용자의 몫이다."
...
import { daysBetweenKeys, isValidDateKey, shiftDateKey, toDateKey } from './date'
import type {
  ActivityLevel,
  CalcOptions,
  DietGoal,
...
/** 기초대사량 (Mifflin-St Jeor) — 명세 105~107줄. 반올림하지 않는다 */
...
/** 일일 총 소비 열량 — 명세 109줄. 여기서 반올림한다 */
export function calcTDEE(bmr: number, activityLevel: ActivityLevel): number {
  return Math.round(bmr * ACTIVITY_FACTORS[activityLevel])
}

/** 목표 달성에 필요한 하루 조정 열량 — 명세 121~123줄. 항상 0 이상 */
...
/** 주당 예상 변화 속도 (kg/주, 소수 2자리) — 명세 185줄 */
...
export function calcMacros(params: {
  ...
  // 목표 칼로리와 달리 여기서 음수는 의미를 갖지 않는다. 0에서 멈추고 사실을 알린다.
  if (carbsG < 0) {
    carbsG = 0
...
/**
 * F1 전체 계산. 화면과 다른 기능(F2·F3·F5·F7)이 쓰는 진입점.
 */
export function calcAllTargets(profile: Profile, options: CalcOptions = {}): Targets {
  ...
  // ── 유지 목표 판정 ────────────────────────────────────────────
  // 목표 체중 = 현재 체중이면 무엇을 골랐든 유지로 처리한다 (명세 194줄).
  const hasGoalInputs =
  ...
  const weightMatchesTarget = hasGoalInputs && profile.targetWeightKg === profile.weightKg
  ...
  const targetDate = isValidDateKey(profile.goalStartDate)
    ? shiftDateKey(profile.goalStartDate, goalDurationDays)
    : null
  const rawRemaining = targetDate != null ? daysBetweenKeys(todayKey, targetDate) : null
  const remainingDays = rawRemaining == null ? null : Math.max(0, rawRemaining)

  // 몸무게를 갱신하면 남은 기간으로 다시 나눈다 (명세 196줄).
  // 기간이 지났으면 0으로 나눌 수 없으므로 원래 기간을 분모로 삼아 값을 고정하고
  // 새 목표 설정을 유도한다 (명세 197줄).
  ...
  const baseTargetCalories =
    effectiveGoal === 'lose' ? tdee - dailyAdjustmentKcal : tdee + dailyAdjustmentKcal
